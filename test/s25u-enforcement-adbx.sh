...
#  - Uses ADB shim: prefers `adbx` (bound wrapper), falls back to `adb`
...
#  - Revert script auto-detects adbx/adb at runtime
# ==============================================================================
...
if [[ -n "${ADB}" && -x "${ADB}" ]]; then
  ...
  die "Missing dependency: adb (or adbx wrapper)."
fi
...
pick_device() {
  # Respect ANDROID_SERIAL if set; otherwise choose the first authorized device.
  if [[ -n "${ANDROID_SERIAL:-}" ]]; then
    SERIAL="$ANDROID_SERIAL"
  else
    mapfile -t devs < <("$ADB" devices | awk 'NR>1 && $2=="device"{print $1}')
    case "${#devs[@]}" in
      0) die "No device. Connect via USB, enable USB debugging, and accept RSA prompt." ;;
      1) SERIAL="${devs[0]}" ;;
      *) printf "[!] Multiple devices: %s\n" "${devs[*]}" >&2
         die "Set ANDROID_SERIAL to target a device (or use your adbx wrapper bound to one)." ;;
    esac
  fi

  # Positively confirm device state
  state="$("$ADB" -s "$SERIAL" get-state 2>/dev/null || true)"
  case "$state" in
    device) export ANDROID_SERIAL="$SERIAL" ;;
    unauthorized) die "Device '$SERIAL' is UNAUTHORIZED. Revoke USB debugging & re-accept RSA on device." ;;
...
# ---------- Preset profile (env may override) ---------------------------------
PROFILE="${PROFILE:-}"   # maxperf|balanced|battery|""
set_if_unset() { local k="$1" v="$2"; eval "[[ -z \"\${$k:-}\" ]]" && printf -v "$k" '%s' "$v"; }
apply_profile() {
  case "${PROFILE}" in
    maxperf)
...
# Permissions / AppOps (manual lists)
...
# Revert script header: pick the same binary if available, else fallback to adb
{
  printf "#!/usr/bin/env bash\nset -Eeuo pipefail\n"
  printf "ADB_BIN=%q\n" "$ADB_PATH"
  printf "command -v \"$ADB_BIN\" >/dev/null 2>&1 || ADB_BIN=adb\n"
} > "$REENABLE_SCRIPT"
chmod +x "$REENABLE_SCRIPT"

# ---------- Helpers (use $ADB everywhere) -------------------------------------
...
ensure_revert(){
  ...
  if [[ -z "$old" || "$old" == "null" ]]; then
    echo '"$ADB_BIN" shell settings delete '"$scope $key" >> "$REENABLE_SCRIPT"
  else
    # shell-escape the old value within printf and the echo payload
    printf '"$ADB_BIN" shell settings put %s %s %q\n' "$scope" "$key" "$old" >> "$REENABLE_SCRIPT"
  fi
...
ensure_dc_revert(){
  ...
  if [[ -z "$old" || "$old" == "null" ]]; then
    echo '"$ADB_BIN" shell device_config delete '"$ns $key" >> "$REENABLE_SCRIPT"
  else
    printf '"$ADB_BIN" shell device_config put %s %s %q\n' "$ns" "$key" "$old" >> "$REENABLE_SCRIPT"
  fi
...
ensure_appops_revert(){
  ...
  echo '"$ADB_BIN" shell cmd appops reset '"$pkg" >> "$REENABLE_SCRIPT"
...
if [[ -n "$DATA_SAVER" ]]; then
  case "$DATA_SAVER" in
    on|off)
      ...
      echo '"$ADB_BIN" shell cmd netpolicy set restrict-background '"${current:-false}" >> "$REENABLE_SCRIPT"
      if [[ "$DRY_RUN" == "1" ]]; then
  ...
  for pkg in $BACKGROUND_SYNC_BLACKLIST; do
    ...
    if [[ "$DRY_RUN" == "1" ]]; then
      ...
      if "$ADB" shell cmd netpolicy add restrict-background-blacklist "$uid" >/dev/null 2>&1; then
        ...
        echo '"$ADB_BIN" shell cmd netpolicy remove restrict-background-blacklist '"$uid" >> "$REENABLE_SCRIPT"
      else
...
if [[ -n "$ART_ACTION" || -n "$ART_SPEED_PROFILE_PKGS" ]]; then
  ...
  grep -q "cmd package compile --reset -a" "$REENABLE_SCRIPT" || echo '"$ADB_BIN" shell cmd package compile --reset -a' >> "$REENABLE_SCRIPT"

  sec_flag=()
  [[ "$ART_INCLUDE_SECONDARY" == "1" ]] && sec_flag+=(--secondary-dex)

  case "$ART_ACTION" in
    bg)                run "$ADB" shell cmd package bg-dexopt-job ;;
    speed-profile-all) run "$ADB" shell cmd package compile -m speed-profile "${sec_flag[@]}" -a ;;
    speed-all)         run "$ADB" shell cmd package compile -m speed -f "${sec_flag[@]}" -a ;;
    "" ) ;;
    *  ) warn "Unknown ART_ACTION '$ART_ACTION'";;
  esac
  for p in $ART_SPEED_PROFILE_PKGS; do run "$ADB" shell cmd package compile -m speed-profile "${sec_flag[@]}" "$p" || true; done
...
case "$PHANTOM_MODE" in
  relaxed)
    echo '"$ADB_BIN" shell cmd device_config set_sync_disabled_for_tests none' >> "$REENABLE_SCRIPT"
...
# ---------- App Standby Buckets (space-separated pairs: pkg:bucket) -----------
...
# ---------- Optional app-ops blocking (manual list) ----------
...
if [[ "${INCLUDE_SAMSUNG_HINTS,,}" == "on" ]]; then
  {
    echo "# HINT: If you want to soft-disable Samsung Wallet/Pass/TVPlus/News by CSC here,"
    echo "# pair this file with s24u-degoogle.sh --include-samsung on and --keep-samsung as needed."
  } >> "$REENABLE_SCRIPT"
fi
